...
static struct codec_base ffmpeg_image_codec = {
	.sub_class = ES_CODEC_CLASS_IMAGE,
	.attr = {
		.codec_class = ES_CODEC_CLASS_VIDEO,
		.codec_fmt.image_fmt = ES_VIDEO_ENCODE_FMT_UNKNOW,
...
es_error_t es_init_ffmpeg_image_codec()
{
	es_error_t ret = ES_SUCCESS;
	
	ret = codec_base_register(&ffmpeg_image_codec);
	return ret;
